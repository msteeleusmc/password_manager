...
class UserMenu(Menu):
    def __init__(self, parent):
        ...
        fileMenu.add_command(label="Password Generator", underline=1,
                             command=lambda: parent.switch_frame(select_Generator))
...
class select_Generator(Frame):
    def __init__(self, master):
        ...
        Checkbutton_style2.configure('checkbutton2.TCheckbutton', background=bg_color, foreground=fg_color, padx=10,
                                     pady=10, font='Helvetica 15 bold')

        self.mix_but = ttk.Checkbutton(f2, text="Mix Password", onvalue=1, offvalue=0, variable=select,
                                       style='checkbutton2.TCheckbutton')
        self.mix_but.grid(row=1, column=0, sticky='w')

        self.aplha_num_but = ttk.Checkbutton(f2, text="Alpha Numerical Password", onvalue=2, offvalue=0,
                                             variable=select, style='checkbutton2.TCheckbutton')
        self.aplha_num_but.grid(row=2, column=0, sticky='w')

        self.num_but = ttk.Checkbutton(f2, text="Numerical Password", onvalue=3, offvalue=0, variable=select,
                                       style='checkbutton2.TCheckbutton')
...
class Pass_Generator(Frame):
    def __init__(self, master):
        ...
        T = Text(frame, bg=bg_color, fg='slateblue', font='Helvetica 15 ', width=50, height=17,
                 yscrollcommand=Scrollbar1.set, selectbackground='hotpink', selectforeground='snow')
        T.pack(pady=30, )
        ...
        ttk.Button(fa, text='Back', style="TButton", command=lambda: master.switch_frame(select_Generator)).grid(row=0,
                                                                                                                 column=0)
        ttk.Button(fa, text='Login', style="TButton", command=lambda: master.switch_frame(Login_page)).grid(row=0,
                                                                                                            column=1,
                                                                                                            padx=20)
